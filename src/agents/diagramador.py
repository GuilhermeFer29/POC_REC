...
def create_diagramador_agent(settings: Settings) -> Agent:
    return Agent(
        ...
        instructions=[
            ...
            "- Nome do produto e marca (se disponível)",
            "- URL da imagem original do produto (se disponível)",
            "- Lista de URLs de imagens dos passos",
            ...
            "1. **PRIMEIRO SLIDE**: Imagem original do produto (se fornecida)",
            "   - Legenda: 'Produto: [Nome] [Marca]'",
            "2. **SLIDES SEGUINTES**: Uma imagem para cada passo do modo de preparo",
            "   - Legenda: 'Passo X: [descrição do passo]'",
            "",
            "### ELEMENTOS DA PÁGINA:",
            "1. Título da receita (incluindo marca se disponível)",
            "2. CARROSSEL DE IMAGENS usando inputs radio e labels",
            "3. Seção de ingredientes formatada",
            "4. Seção de modo de preparo numerado",
            "",
            "## REGRAS IMPORTANTES:",
            "- A IMAGEM DO PRODUTO deve ser o PRIMEIRO slide do carrossel",
            "- Use EXATAMENTE as URLs de imagens fornecidas",
            "- NÃO USE JAVASCRIPT - use apenas CSS puro com inputs radio",
            "- Inclua CSS inline no HTML (tag <style>)",
            "- Crie um input radio e um label/dot para CADA imagem",
